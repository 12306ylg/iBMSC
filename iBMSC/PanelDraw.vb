...
Partial Public Class MainWindow
    ...
    Private bufferlist As New Dictionary(Of Integer, BufferedGraphics)
    Private rectList As New Dictionary(Of Integer, Rectangle)
    Private Function GetBuffer(xIndex As Integer, DisplayRect As Rectangle)
        ...
        Else
            If bufferlist.ContainsKey(xIndex) Then
                ...
                Dim unused1 = bufferlist.Remove(xIndex)
                Dim unused = rectList.Remove(xIndex)
            End If
    ...
    Private Sub DrawDragAndDrop(xIndex As Integer, e1 As BufferedGraphics)
        If UBound(DDFileName) > -1 Then
            ...
            e1.Graphics.DrawString(Join(DDFileName, vbCrLf), Font, xBrush, spMain(xIndex).DisplayRectangle, xFormat)
        End If
    End Sub
    ...
    Public Function GetColumnHighlightColor(col As Color, Optional factor As Double = 2.0)
    ...
    Private Sub DrawMouseOver(e1 As BufferedGraphics, xTHeight As Integer, xHS As Integer, xVS As Integer)
        ...
        Dim unused1 = e1.Graphics.DrawRectangle(pen, rect.X, rect.Y, rect.Width - 1, rect.Height - 1)
        ...
        If ModifierMultiselectActive() Then
            For Each note In Notes
                If IsNoteVisible(note, xTHeight, xVS) AndAlso IsLabelMatch(note, KMouseOver) Then
                    ...
                    Dim unused = e1.Graphics.DrawRectangle(pen, nrect.X, nrect.Y, nrect.Width - 1, nrect.Height - 1)
                End If
            Next
        End If
    ...
    Private Sub DrawNote(ByVal sNote As Note, ByVal e As BufferedGraphics, ByVal xHS As Long, ByVal xVS As Long, ByVal xHeight As Integer) ', Optional ByVal CheckError As Boolean = True) ', Optional ByVal ConnectToIndex As Long = 0)
        ...
        ' Label
        Dim unused = e.Graphics.DrawString(IIf(IsColumnNumeric(sNote.ColumnIndex), sNote.Value / 10000, xLabel),
                              vo.kFont, xBrush2,
        ...
        If ErrorCheck AndAlso sNote.HasError Then e.Graphics.DrawImage(My.Resources.ImageError,
                                                            HorizontalPositiontoDisplay(nLeft(sNote.ColumnIndex) + (GetColumnWidth(sNote.ColumnIndex) / 2), xHS) - 12,
